...
async function markAsCompleted() {
    ...
    if (isCL001) {
        ...
        if (!confirm('Mark this appointment as completed?\n\nThis will accept the linked PN case.')) return;
        await completeAppointmentSimple();
    } else {
        // Non-CL001 clinic - show PT Assessment modal
        showPTAssessmentModal(currentAppointmentId);
    }
}
...
function showPTAssessmentModal(appointmentId) {
    const modalHtml = `
...
async function submitPTAssessmentForAppointment(appointmentId) {
    ...
    try {
        ...
        showAlert('Appointment completed with PT assessment', 'success');
