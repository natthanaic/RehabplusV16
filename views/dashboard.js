...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadClinics();
    loadCases();
    loadDashboardSummary();  // NEW: Load summary statistics
...
// Step 1: Check if the API returned ANYTHING. 
    // If the original 'cases' array is empty, then there are truly no cases.
    if (cases.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" class="text-center">No cases found</td></tr>';
        return;
    }
    
    // Step 2: If we have cases, NOW we apply the filter to hide "CANCELLED" on the "All Status" view.
    const statusFilter = document.getElementById('filter-status').value;
    let casesToDisplay = cases;

    if (statusFilter === '') {
        casesToDisplay = cases.filter(pnCase => pnCase.status !== 'CANCELLED');
    }

    // Step 3: Check if the *filtered* list is empty.
    // This handles your case: the API returned cases, but they were all "CANCELLED"
    // and we are on the "All Status" view.
    if (casesToDisplay.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" class="text-center">No cases found</td></tr>';
        return;
    }
    
    tbody.innerHTML = casesToDisplay.map(pnCase => `
        <tr>
            <td>${escapeHtml(pnCase.hn)}</td>
            <td>${escapeHtml(pnCase.first_name)} ${escapeHtml(pnCase.last_name)}</td>
            <td><span class="badge bg-secondary">${escapeHtml(pnCase.pn_code)}</span></td>
            <td>${escapeHtml(truncateText(pnCase.diagnosis, 50))}</td>
            <td>${escapeHtml(truncateText(pnCase.purpose, 50))}</td>
